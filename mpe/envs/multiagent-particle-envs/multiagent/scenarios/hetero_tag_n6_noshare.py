...
class Scenario(BaseScenario):
    ...
    def make_world(self):
        ...
        world.agent_size = 0.05
        world.adversary_size = 0.075
        world.landmark_size = 0.2
        # add agents
        ...
        for i, agent in enumerate(world.agents):
            ...
            agent.size = world.adversary_size if agent.adversary else world.agent_size
        ...
        self.n_adv_visible_agent = 1
        self.n_adv_visible_landmark = 3
        self.n_adv_visible_adv = 5
        world.n_adv_visible_agent = self.n_adv_visible_agent
        world.n_adv_visible_landmark = self.n_adv_visible_landmark
        world.n_adv_visible_adv = self.n_adv_visible_adv
        world.n_visible_agent = self.n_visible_agent
        world.n_visible_landmark = self.n_visible_landmark
        world.n_visible_adv = self.n_visible_adv
    ...
    def adversary_reward(self, agent, world):
        ...
        dist_to_agents = [np.sqrt(np.sum(np.square(a.state.p_pos - agent.state.p_pos))) for a in agents]

        rew -= 0.1 * min(dist_to_agents)
        if agent.collide:
            n_collide = (np.array(dist_to_agents) < self.collide_th).sum()
        rew += 10 * n_collide
